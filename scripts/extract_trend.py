    ...
    for lake in lakes_list:
        ...
        if os.path.exists(product_stats_path):
            ...
            for band in bands:
                ...
                baseline = decomposition.observed.nsmallest(3, 'values').mean()[0]
                baselines.append(baseline)
                idx = decomposition.observed['values'] > baseline * 1.5
                over_baseline = decomposition.observed['values'][idx].index.strftime("%Y-%m-%d").tolist()
                baseline_exceeded.append(over_baseline)
                ...
                high = decomposition.resid['values'][idx].index.strftime("%Y-%m-%d").tolist()
